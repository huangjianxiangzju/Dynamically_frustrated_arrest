...
plt.rcParams.update({
    'grid.alpha': 0.8,      # 透明度
    'grid.color': '0.9',   # 更浅的颜色
    'grid.linewidth': 1   # 可选：细一点
})
...
for idx, (group_title, group_domains) in enumerate(groups.items()):
    ...
    for c_idx, domain in enumerate(group_domains):
        ...
        ax.plot(display_states, y_values, marker=d_marker, markersize=10, linewidth=2.5, 
    ...
    ax.set_title(group_title, fontsize=20, pad=15)
    ...
    if idx == 0:
        ...
        ax.set_ylabel('Median RMSF (Å)', fontsize=18)
    
    # Legend settings: inside the plot, no circling/frame
    legend = ax.legend(fontsize=18, loc='best', frameon=False)
